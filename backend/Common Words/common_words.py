...
# CONFIGURATION
INPUT_DIR = "."  # Folder containing your .txt files
OUTPUT_FILE = "common_words_data.txt"
OVERLAP_FILE = "common_words.txt"

EXCLUDED_FILES = {OUTPUT_FILE, OVERLAP_FILE}

# Clean and tokenize text
def tokenize(text):
    text = text.lower()
    return set(re.findall(r'\b[a-záéíóúüñ]+\b', text))  # unique words per file
...
# Write just the overlapping words to a plain list
def write_overlap_file(word_counts, overlap_file):
    overlapping_words = [word for word, count in word_counts.items() if count >= 2]
    with open(overlap_file, "w", encoding="utf-8") as f:
        for word in sorted(overlapping_words):
            f.write(f"{word}\n")
...
def main():
    ...
    categories = categorize_frequencies(word_counts, total_files)
    write_output(categories, OUTPUT_FILE)
    write_overlap_file(word_counts, OVERLAP_FILE)
    print(f"Categorized word report saved to: {OUTPUT_FILE}")
    print(f"Overlapping words saved to: {OVERLAP_FILE}")
